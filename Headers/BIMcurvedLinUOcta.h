...
{   
    ...
    ThreeDVector x0;
        ...
        // size must be 6 to begin with an octahedron
    mesh(ThreeDVector * pts, int size)  
    {
        ...
        // for octahedron, 8 elements are created at the beginning:
        element[0] = ThreeDVector(0, 1, 4);
        element[1] = ThreeDVector(1, 2, 4);
        element[2] = ThreeDVector(2, 3, 4);
        element[3] = ThreeDVector(3, 0, 4);
        element[4] = ThreeDVector(1, 0, 5);
        element[5] = ThreeDVector(2, 1, 5);
        element[6] = ThreeDVector(3, 2, 5);
        element[7] = ThreeDVector(0, 3, 5);
        ...
        for (int iElem = 0; iElem < element.size(); iElem++)
        {
          ...
          //  ThreeDVector delU = uS[iElem] - uS[eIndx];
            ...
            for(int iQuad=0; iQuad < w.size(); iQuad++)
            {
                ...
                double modR = r.norm();
                ThreeDVector nDotuDotT = r*( -3.0*(normalVector.dot(r)*delU.dot(r))/(4.0*M_PI*pow(modR, 5.0)) );
